...
/*   Updated: 2024/11/22 19:54:45 by nyoshimi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char* get_pixel(t_map *map,t_vector pos,t_plane *plane)
{
	...
	if(plane->type == 1 || plane->type == 3)
	{
		...
		y = 64 - pos.z;
		printf("x = %d, y = %d\n",x,y);
		return (&map[plane->type -1].image->addr[y*map->image[plane->type-1].line_length + x*(map->image[plane->type-1].bits_per_pixel / 8)]);
	}
	else if(plane->type == 2 || plane->type == 4)
	{
