...
/*   Updated: 2024/11/28 20:16:00 by nyoshimi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_step_var(t_tool *tool)
{
	...
	{
		tool->step_x = -1;
		tool->to_near_x = (tool->player->pos.x - tool->focus_x)
			* tool->to_next_x;
	}
	else
	{
		tool->step_x = 1;
		tool->to_near_x = (tool->focus_x + 1.0 - tool->player->pos.x)
			* tool->to_next_x;
	}
	if (tool->ray_y < 0)
	{
		tool->step_y = -1;
		tool->to_near_y = (tool->player->pos.y - tool->focus_y)
			* tool->to_next_y;
	}
	else
	{
		tool->step_y = 1;
		tool->to_near_y = (tool->focus_y + 1.0 - tool->player->pos.y)
			* tool->to_next_y;
	}
}
...
void	set_distance(t_tool *tool)
{
	...
	tool->wall_x -= floor((tool->wall_x));
	tool->img_x = (int)(tool->wall_x
			* (double)(tool->map->image[tool->side].width));
...
void	set_pixel(t_tool *tool, int x)
{
	...
	{
		if (y < tool->pix_start)
			set_color(&tool->vars->img.addr[y * tool->vars->img.line_length + x
				* tool->vars->img.bits_per_pixel / 8], tool->map->ceiling);
		else if (y < tool->pix_end)
		{
			tool->img_y = (y - tool->pix_top) / ((double)(tool->pix_end
						- tool->pix_top)
					/ (double)tool->map->image[tool->side].height);
			tool->color = get_color(
					&tool->map->image[tool->side].addr[tool->img_y
					* tool->map->image[tool->side].line_length + tool->img_x
					* (tool->map->image[tool->side].bits_per_pixel / 8)]);
			set_color(&tool->vars->img.addr[y * tool->vars->img.line_length + x
				* tool->vars->img.bits_per_pixel / 8], tool->color);
		}
		else
			set_color(&tool->vars->img.addr[y * tool->vars->img.line_length + x
				* tool->vars->img.bits_per_pixel / 8], tool->map->floor);
		y++;
	}
}
